...
export const POST = async (
  ...
  try {
    ...
    if (action === 'like') {
      ...
      return NextResponse.json(
        { data: updatedProduct, message: 'Product liked successfully!' },
        { status: 200 },
      );
    }
    ...
    if (action === 'unlike') {
      ...
      return NextResponse.json(
        { data: updatedProduct, message: 'Product unliked successfully!' },
        { status: 200 },
      );
    }
    ...
    return NextResponse.json(
      { error: 'Failed to handle like action', details: error },
      { status: 500 },
    );
  }
};
