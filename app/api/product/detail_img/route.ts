...
export const POST = async (req: Request) => {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to upload and add image', details: error },
      { status: 500 },
    );
  }
};
...
export const DELETE = async (req: Request) => {
  try {
    ...
    if (!productId || !imageUrlToRemove) {
      return NextResponse.json(
        { error: 'Product ID and image URL to remove are required' },
        { status: 400 },
      );
    }
    ...
    const updatedDetailImg = product.detail_img.filter(
    ...
    return NextResponse.json(
      { error: 'Failed to remove image', details: error },
      { status: 500 },
    );
  }
};
